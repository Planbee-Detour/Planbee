...
- **개발 빌드에서는 RN LogBox 알림 배너가 화면 하단 요소를 덮는다.** JS 경고가 한 건이라도 나면
  RN 이 화면 맨 아래 [10,787][392,835] 에 배너를 띄우고, 그 자리는 `status-go-login` 버튼과
  하단 탭바(`tab-My`)의 **중심**이 있는 곳이다. Maestro 는 요소 중심을 누르므로 탭이 배너에 먹힌다.
  접근성 트리에는 `!, Open debugger to view warnings.` 로 올라온다 — 실패 스크린샷에서
  앱의 Toast 로 **오해하기 쉬우니 트리를 먼저 확인한다.** 플로우는 배너가 있을 때만
  닫기(X)를 `'96%,93%'` 로 눌러 치운다. 릴리스 빌드에는 LogBox 가 없어 배너도 없다.
- **iOS 시스템 다이얼로그는 "있을 때만" 처리한다.** "암호를 저장하겠습니까?" 는 매번 뜨지 않는다
  (같은 실행에서 한 플로우엔 뜨고 다른 플로우엔 안 떴다). 그래서 좌표를 무조건 누르지 않고
  `tapOn: text: '지금 안 함'` + `optional: true` 를 `retry` 로 감싼다 — 없으면 건너뛰고,
  늦게 뜨면 다음 회차가 닫는다. 이 다이얼로그는 접근성 트리에 올라온다.
  반면 비밀번호 칸의 "강력한 암호를 사용하겠습니까?" **시트**는 트리에 없어 좌표뿐이다.
- **Maestro 의 `index` 는 계층 순서가 아니라 화면 y 위치 순서다.** 같은 문자열이 여러 개일 때
  `index: 1` 은 "두 번째로 나타난 요소" 가 아니라 "위에서 두 번째 요소" 다. 레이아웃이 바뀌면
  조용히 다른 것을 누른다 — 실제로 로그아웃 알럿에서 알럿 뒤에 깔린 설정 행을 눌렀다.
  가능하면 `id:` 로, 없으면 `rightOf` / `below` 처럼 **구조**로 짚는다.
- **`appId` 는 iOS 번들 ID 다** (`org.reactjs.native.example.Planbee`).
  ...
  임의로 맞추지 않았다. `docs/features/auth/status.md` 의 `ASK` 10 참조.

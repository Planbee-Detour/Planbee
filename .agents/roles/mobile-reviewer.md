...
- **플랫폼 (M-19·M-20)**: iOS 전용 API·네이티브 모듈을 안드로이드 대체 경로 없이 공용 코드에 넣었는가.
  `Platform.select` 에 `android`/`default` 키가 비었는가. `.ios.tsx`/`.android.tsx` 로 나눈 파일에
  사유 주석이 없거나 두 구현이 어긋나는가. iOS 에서만 동작하는 경로를 분기 없이 남겼는가.
  배포에 걸리는 결정(권한·서명·최소 버전·SDK 도입)을 사람에게 묻지 않고 코드로 정해 버렸는가.

...
- 엔티티를 추가·변경하면 **같은 커밋에서** Flyway 마이그레이션을 넣는다. 없으면 서버가 뜨지 않는다. (S-14)
- **조회 방식을 먼저 고른다** (S-23): 단순 조회는 Spring Data JPA 메서드,
  조건이 실행 시점에 달라지거나(필터·검색·정렬 조합) 다중 조인·프로젝션이 필요하면 QueryDSL.
  단순 조회를 QueryDSL 로 다시 쓰지 않고, 동적 조건을 JPQL 문자열로 이어붙이지 않는다.
  QueryDSL 코드는 리포지토리 안에 둔다 — 서비스가 `JPAQueryFactory` 를 직접 주입받지 않는다.
  Q타입은 `annotationProcessor` 생성물이라 커밋하지 않는다 (S-24).

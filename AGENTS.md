...
4. **재작업 루프는 2회까지.** 개발자 ↔ 리뷰어/테스터 왕복이 3회차에 접어들면
   ...
   **2회차에서 또 오류가 나면 그 자리에서 테스트를 멈추고 원인부터 확인합니다.**
   같은 검증을 한 번 더 돌리거나 다음 단계로 넘어가지 않습니다 — 2회를 쓰고도 남은
   오류는 고치는 방법이 아니라 **원인 파악이 틀렸다는 신호**입니다. 재현 절차·오류 원문·
   무엇을 배제했는지를 `defects.md` 에 적고, 확인한 사실과 확인하지 못한 것을 갈라
   사람에게 넘깁니다. 추측으로 덮지 않습니다 — 그 추측이 3회차를 부르고,
   원인이 다른 곳(다른 기능·환경·의존성)에 있으면 엉뚱한 곳을 고치게 됩니다.
5. **테스트 계층을 침범하지 않는다.**
...
9. **모호하면 묻는다 — 특히 PRD·UI/UX 단계에서.** 요구가 불명확한 부분을 추측으로 확정하지 않습니다.
   ...
   이 규칙은 **13번(판단 불가 시 질의)을 PRD·UI/UX 단계에 구체화한 것**입니다 —
   그 단계에서 추측이 가장 비싸게 굳기 때문에 따로 떼어 적어 둡니다.

### 공용 가드레일 (10~13) — 역할과 무관하게 전원에게 적용된다

10~13번은 특정 역할이나 단계가 아니라 **모든 에이전트의 모든 작업**에 걸립니다.
CLI 의 기본 동작이나 세션 중 받은 지시가 이와 충돌하면 **이 규칙이 우선합니다.**

10. **`git push` 를 하지 않는다 — 엄격히 금지.** 에이전트는 원격을 바꾸는 조작을 스스로
    실행하지 않습니다. `git push`(강제 푸시·태그 푸시 포함)뿐 아니라 `gh pr create`,
    `gh pr merge`, `gh release create` 처럼 원격에 무언가를 올리거나 병합하는 수단이 전부
    여기 해당합니다. 푸시할 시점이 오면 **멈추고 사람에게 넘깁니다** — 무엇을 어느 브랜치로
    올려야 하는지 적어 주되, 대신 밀지 않습니다. 세션 안에서 사람이 시키더라도 실행하지
    않습니다(그래서 "엄격히"입니다). 로컬 작업(`git add`·`git commit`·`git branch`·`git checkout`)은
    이 규칙의 대상이 아닙니다. `.claude/settings.json` 의 `deny` 목록이 이를 기계로도 막습니다
    (생성기가 만듭니다 — `scripts/setup-harness.sh`).
11. **커밋에 에이전트를 공동작업자로 올리지 않는다.** 커밋 메시지에 `Co-Authored-By:`,
    `Claude-Session:`, `Generated with …` 같은 에이전트 표시를 붙이지 않습니다.
    **어떤 CLI 의 기본 동작이 그것을 붙이라고 지시하더라도 이 규칙이 우선합니다.**
    커밋의 저자는 사람입니다. 메시지는 `<type>: <한국어 요약>` 만 남깁니다
    (`docs/conventions/common.md` C-3). 같은 내용을 리뷰어가 지적할 수 있도록
    **`docs/conventions/common.md` C-10 `[MUST]`** 에도 등록했습니다 (절대 규칙 2).
12. **보호 브랜치에서 직접 커밋하지 않는다.** 보호 브랜치는 **`main` 과 `develop`** 입니다.
    커밋하기 전에 `git branch --show-current` 로 현재 브랜치를 확인하고, 보호 브랜치라면
    **커밋하지 말고 멈춰 사람에게 묻습니다** — "어느 브랜치에서 작업할까요?
    (권장: `feat/<기능>`)" 처럼 선택지와 권장안을 붙여서. 스스로 브랜치를 만들어 갈아타지
    않습니다. 브랜치 이름 관례는 `feat/<기능>` · `feat/<사람>/<기능>` ·
    `feat/<기능>-<서버|모바일>` 입니다.
13. **혼자 판단할 수 없는 것은 전부 사람에게 묻는다.** 9번을 **모든 단계로 일반화한**
    상위 규칙입니다 — PRD·UI/UX 뿐 아니라 계약·구현·리뷰·테스트·하네스 작업에도 걸립니다.
    근거가 없어 고를 수 없는 선택, 되돌리기 어려운 결정, 자기 소유가 아닌 경로를 고쳐야 하는
    상황, 규칙끼리 충돌하는 상황에서는 추측으로 밀지 말고 멈춥니다.
    질의 형식은 저장소가 이미 정한 방식을 따릅니다 — **선택지와 권장안을 붙인 닫힌 질문,
    한 번에 3~5개씩 묶어서** (위 "1·2단계는 소크라테스식으로 묻는다").
    파이프라인 작업 중이라면 `status.md` 에 `ASK` 와 질문을 적고 넘깁니다.
    사람이 "알아서 해줘" 라고 위임하면 그때는 정하되, 무엇을 어떻게 가정했는지 문서에 남깁니다.

...
기능 하나를 통째로 맡기려면 에이전트에게 이렇게 말하면 됩니다.

```
일정 생성 기능을 개발할 거야. 순서대로 진행해줘.
```

PRD → UI/UX 명세 → **pen 시각화** → API 계약 → 백엔드 → 프론트 → 테스트까지 순서대로 진행합니다.
(Claude Code 에서는 `/feature <기능 설명>` 도 같은 요청입니다.)
PRD 와 UI/UX 단계에서는 에이전트가 모호한 부분을 임의로 정하지 않고 선택지를 붙여 되묻습니다 —
자세한 규칙은 `AGENTS.md` 의 "기능 개발 요청 — 순서대로 진행해줘" 에 있습니다.

클론 직후 세팅은 `make install` 한 번이면 되고, 쓰는 CLI 에게 **"환경에 맞게 세팅해줘"**
라고 시켜도 됩니다 — 에이전트가 정본에서 자기 CLI 가 읽는 파일을 생성합니다.
절차와 금지사항은 `AGENTS.md` 의 "환경 세팅 — 정본에서 생성한다" 에 있습니다.

...
- 사용하는 HTTP 상태는 `400 / 401 / 403 / 404 / 405 / 409 / 429 / 500` 로 제한한다. 그 외가 필요하면 tech-lead 와 합의한다.
- **`429` 를 쓰는 응답에는 `Retry-After` 헤더(초, 정수)를 반드시 함께 낸다.** RFC 9110 표준 헤더다.
  다만 **모바일은 이 헤더로 분기하지 않는다** — 화면에 표시할 값이 필요하면 그 값을 본문 필드로
  따로 내린다 (C-8). 예: `auth` 의 잠금 응답은 `Retry-After: 540`(실제 초)과
  `lock_remaining_minutes: 9`(올림·최소 1분)를 함께 내리고, 화면은 후자만 렌더한다.
  헤더는 표준 준수와 자동화 클라이언트를 위한 것이고, 사람이 읽는 문구의 근거는 본문 필드다.
  - 2026-08-26 추가. 근거: `auth` 의 로그인 시도 초과(AC-17)와 가입 IP 레이트 리밋이
    "권한 없음"(403)도 "요청이 잘못됨"(400)도 아닌 **"지금은 말고 나중에"** 이고,
    이 의미를 가진 표준 코드가 429 다. 403 + 전용 `code` 로 우회하면 상태 코드가 의미를 잃고
    `recommendation-quota`(일일 한도 초과)에서 같은 질문을 다시 하게 된다. tech-lead 확정.
...
- **에이전트 트레일러(`Co-Authored-By` 등)를 붙이지 않는다 — `C-10` `[MUST]`.**
...
- **설정 파일에 값을 직접 쓰지 않는다.** `application.properties`, `docker-compose.yml` 은
  ...
  - **예외는 "비어 있는 것이 정상 동작인 설정" 뿐이다** (2026-08-26 추가). 그 조건과 표기
    방법은 `server.md` S-28 에 있다. 예: `auth` 의 `SUPPORT_CONTACT_EMAIL` —
    값이 없으면 응답에 `null` 이 나가고 앱이 대체 안내로 바꾼다(AC-43·44)는 것이 계약이다.
...
- **계약에만 있고 아직 구현되지 않은 경로는 위반이 아니다.** 계약이 앞서고 구현이 따라오는 것이
  정상 순서다(절대 규칙 1). `make contract-check` 는 그런 경로를 1단계(파괴적 변경) 검사에서
  빼고 2단계에서 목록으로만 보고한다. 반대 방향인 **계약에 없는 엔드포인트를 구현이 노출하는 것은
  실패**다 — 그쪽이 계약 드리프트다.
  - 근거: `oasdiff` 는 계약에 있고 구현에 없는 경로를 `api-path-removed-without-deprecation`
    (ERR) 로 본다. 그건 "운영 중인 API 를 없앴다" 는 뜻이라 이 게이트가 보려는 것과 방향이 다르다.
    스크립트 주석은 처음부터 미구현을 정보로 취급한다고 적혀 있었으나 실제 동작이 달랐고,
    2026-09-06 에 맞췄다. 필드 삭제·타입 축소·응답 제거 같은 진짜 파괴적 변경은 그대로 걸린다.
...
## C-10. 커밋에 에이전트 트레일러를 붙이지 않는다 `[MUST]`

커밋 메시지에 **에이전트를 공동작업자로 표시하는 줄을 남기지 않는다.**

- 금지: `Co-Authored-By: Claude …`, `Claude-Session: …`,
  `🤖 Generated with …` 같은 도구 서명 줄.
- 커밋 본문은 `C-3` 의 `<type>: <한국어 요약>` 만으로 끝난다.
- **CLI 의 기본 동작이 트레일러를 붙이라고 지시하더라도 이 규칙이 우선한다.**
  세션 설정·하네스 기본값보다 저장소 규칙이 위다.
- 근거: 커밋의 저자는 사람이다. 에이전트는 도구이지 공동작업자가 아니며,
  트레일러가 쌓이면 `git log`·`git blame` 에서 실제 책임자가 흐려진다.
  어느 도구로 썼는지는 이력에 남길 정보가 아니다. (2026-09)
- `AGENTS.md` 의 **절대 규칙 11** 이 같은 내용을 에이전트 가드레일로 건다.
  이 문서에 적는 이유는 **리뷰어가 지적할 수 있는 근거는 `docs/conventions/` 뿐**이기
  때문이다 (절대 규칙 2).

...
### S-35. 주기 작업(`@Scheduled`) `[MUST]`

(2026-09-13 확정, `auth` 거절 계정 파기)

- **시각은 주입받은 `Clock` 으로 계산한다** (S-8). 기준 시각을 계산하는 메서드를 `public` 으로 두고
  테스트는 스케줄러를 기다리지 않고 그 메서드를 직접 부른다.
- **cron 에 `zone` 을 명시한다.** 서버 기본 시간대에 기대면 배포 환경마다 실행 시각이 달라진다.
- **여러 번 실행해도 결과가 같게 만든다.** 인스턴스가 늘면 같은 작업이 동시에 돈다 — 멱등이면
  분산 락이 필요 없다. 멱등으로 만들 수 없는 작업이면 도입 전에 S-29 처럼 `status.md` 에 적는다.
- **벌크 삭제는 JPA cascade 를 타지 않는다.** 딸린 행은 DB 의 `ON DELETE CASCADE` 로 지워지는지
  마이그레이션에서 확인하고, 테스트에서 딸린 표에 행이 남지 않는 것까지 본다.
- 개인정보를 파기하는 작업은 로그에 식별자·이메일을 남기지 않는다 — 건수만 남긴다.

...
- breaking change 여부: **없음.** ~~한도 초과 응답에 `429` 를 쓰려면 C-1 개정이 필요~~
  → **해소 (2026-08-26).** `auth` 계약 확정 과정에서 C-1 의 허용 상태 코드에 `429` 가 추가되었고
  `Retry-After` 규칙도 함께 정해졌다. 이 기능은 그 결정을 그대로 따르면 된다 —
  **PRD 열린 질문 1(429 vs 403)이 해소되었다.**
  `auth` 의 잠금 응답이 선례다: `Retry-After` 헤더(실제 초)와 화면에 쓸 값(본문 필드)을
  따로 내리고, 앱은 본문 필드만 렌더한다 (C-8).
...
`tech-lead` 로 넘길 항목 — ~~PRD 열린 질문 1~~ **해소 (2026-08-26 — 429 확정)**, 4(오류 코드).

...
### 후속 (2026-09-07) — 실제 실행으로 확인한 것 · 요청 1건 추가

- **`text: '비밀번호', index: 1` 은 실제로 입력란을 가리킨다.** 2026-09-07 실행 로그에서
  `Tapping on element: ... hintText=비밀번호, bounds=[32,324][318,348]` 로 확인했다
  (라벨 `Text` 는 그 위에 있다). 뒤집을 필요는 없다. 다만 이유는 "계층 순서" 가 아니라
  **Maestro 가 매칭 요소를 화면 y 위치 순으로 정렬**하기 때문이다 — 라벨이 위, 입력란이 아래다.
  라벨과 입력란의 위아래가 뒤집히거나 사이에 요소가 끼면 조용히 다른 것을 누른다.
  같은 성질 때문에 `auth-03` 의 로그아웃 알럿에서 실제로 **틀린 요소를 눌렀다** (D-E2 함정 7).
- **요청 추가 — 로그아웃 확인 알럿의 버튼에도 잡을 것이 필요하다.**
  설정 화면의 행(`settings-logout`)과 알럿의 확인 버튼이 **같은 문자열 `로그아웃`** 이라
  지금은 `rightOf: text: '취소'` 로 알럿 구조를 짚고 있다. 통과하지만, 알럿 문구
  (`취소` / `로그아웃`)에 묶인 셀렉터라 문구가 바뀌면 깨진다.
  `Alert.alert` 대신 앱 컴포넌트로 만든 확인 다이얼로그를 쓰거나, 두 라벨을 서로 다른 문구로
  두면(예: 행은 `로그아웃`, 확인 버튼은 `로그아웃하기`) 셀렉터가 단순해진다.
  **판단은 mobile-developer / ux-designer 몫이고, 이 항목은 파이프라인을 막지 않는다** —
  현재 플로우는 이 상태로 통과한다.

## D-E2 [Medium] iOS E2E 에서 Maestro 가 앱에 닿지 못하는 함정 (2026-09-02 4가지 + 2026-09-07 3가지)

- 올린 역할: integration-tester (2026-09-02, 2026-09-07 이어 씀)
- 받는 역할: integration-tester (플로우에서 흡수 완료) / 참고: mobile-developer
- 상태: **해소 (플로우 쪽에서 흡수).** 앱을 고칠 항목은 없다.
  2026-09-07 재실행에서 세 가지가 더 나왔고 역시 전부 플로우에서 흡수했다 — 아래 "후속" 절
...
### 후속 (2026-09-07) — develop 머지 뒤 재실행: 3건 실패 → 원인 판별 → **4/4 통과**

- 환경: `make e2e-up`(API `:8080` + PostgreSQL) · Metro 실행 · iPhone 17 Pro / iOS 26.5
  (UDID `D3EB0296-…`) · Maestro **2.10.0**
- 첫 실행 결과: `앱 기동 확인` 통과 / 나머지 3개 실패
  (`가입 신청 → 검토 중 안내`, `로그인 → 홈`, `인증 지속 (재시작) → 로그아웃`)
- 디버그 산출물: `~/.maestro/tests/2026-09-07_192847/`

**세 건 모두 앱·서버·계약의 결함이 아니다.** 실패 시점의 스크린샷과 접근성 트리를 보면
앱과 서버는 전 구간 정상이었다 — 가입은 서버까지 접수돼 **서버가 내려준 안내 문구와 이메일**이
그려져 있었고, 로그인은 성공해 **홈이 완전히 렌더**돼 있었고, 설정 화면에는 서버가 내려준
`approved@e2e.planbee.test` 가 있었다. 실패한 것은 전부 **Maestro 가 요소에 닿는 방법**이다.
위 표의 함정 4가지에 이어 세 가지가 더 있었고, 셋 다 플로우에서 흡수했다.

| # | 증상 | 원인 | 대응 |
|---|---|---|---|
| 5 | `status-go-login` 을 눌렀는데 화면이 그대로. 이어지는 `login-submit` 단언이 17초 뒤 실패 | **RN LogBox 알림 배너**가 버튼을 덮었다. 배너 [10,787][392,835] / 버튼 [17,767][384,819] → **버튼 중심 (200,793) 이 배너 안쪽**이고 Maestro 는 요소 중심을 누른다 | 배너가 있을 때만 닫기(X)를 좌표로 눌러 치운다 (`runFlow: when:`) |
| 6 | 홈이 완전히 그려져 있는데 `tab-My` 단언이 실패 | iOS **"암호를 저장하겠습니까?"** 다이얼로그가 떠 있었다. 열려 있는 동안 그 아래 앱 화면이 접근성 트리에서 사라진다(함정 4와 같은 현상) | `retry` 안에서 `tapOn: text: '지금 안 함'` + `optional: true` → 없으면 건너뛰고, 늦게 뜨면 다음 회차가 닫는다 |
| 7 | 로그아웃 확인 알럿에서 `tapOn: text: '로그아웃', index: 1` 이 **알럿 뒤에 깔린 설정 행**을 눌렀다. 알럿이 열린 채 남아 다음 단언 실패 | Maestro 의 `index` 는 **계층 순서가 아니라 화면 y 위치 순서**다. 알럿 버튼(y 463)이 0, 설정 행(y 526)이 1이었다 | index 를 버리고 알럿 구조로 짚는다 — `tapOn: text: '로그아웃', rightOf: text: '취소'` |

#### 함정 5 — RN LogBox 배너를 앱 Toast 로 오해하지 말 것

실패 스크린샷 하단의 흰 띠는 `LoginScreen` 의 Toast(`mobile/src/shared/ui/Toast.tsx`)처럼 보인다.
**아니다.** 접근성 트리를 뜨면 그 요소는 `!, Open debugger to view warnings.` 이고,
로그인 화면은 아예 마운트되지 않은 상태였다. **개발 빌드에서만 뜨는 RN 개발 도구**이며
릴리스 빌드에는 LogBox 자체가 없다. 로그인 화면에 오류 토스트가 뜬 것이 아니다.

배너의 닫기(X)에는 `testID` 도 `accessibilityLabel` 도 없다 — RN 이 `id` prop(=`nativeID`)만 주고
iOS 에서 그것은 `accessibilityIdentifier` 로 매핑되지 않는다
(`react-native/Libraries/LogBox/UI/LogBoxNotificationDismissButton.js`). 그래서 좌표로 누른다.
실패 스크린샷의 픽셀을 직접 읽어 X 글리프 중심을 **(387.7, 811.3)pt = (96.4%, 92.8%)** 로 재었고,
버튼이 20x20 이므로 `'96%,93%'` 를 쓴다. **이 경로는 실측으로 검증했다** — 디버거로 앱에
`console.warn` 을 하나 넣어 배너를 띄운 뒤 이 좌표를 눌러 배너가 사라지는 것과,
Metro 로그에 `Launching DevTools` 가 늘지 않는 것(= 본문이 아니라 X 를 눌렀다는 뜻)을 확인했다.

배너는 **화면 하단 전체를 가로지르므로 `tab-My` 탭 아이콘(중심 (334,797))도 덮는다.**
그래서 같은 방어를 `auth-02` · `auth-03` 의 마이 탭 진입 앞에도 넣었다.

#### 함정 4 정정 — 저장 다이얼로그는 접근성 트리에 **올라온다**

위 표 4번에 "앱 hierarchy 에 나타나지 않아 좌표로 누를 수밖에 없다" 고 적었는데 **틀렸다.**
`지금 안 함` [57,513][197,561] · `저장` [205,513][345,561] 로 트리에 올라온다.
(트리에 안 나타나는 것은 3번의 "강력한 암호를 사용하겠습니까?" **시트** 쪽이다.)
그래서 `auth-02` · `auth-03` 은 좌표 대신 요소 셀렉터로 누른다. `auth-01` 의 `'31%,61%'` 는
그 자리에서 실제로 통과하고 있어 함께 갈아엎지 않았고, 근거를 플로우 주석에 남겼다.

또 하나: 이 다이얼로그는 **매번 뜨지 않는다.** 같은 실행에서 `auth-03` 은 같은 계정으로
로그인하고도 뜨지 않았다. 그래서 무조건 좌표를 누르면 빈 화면을 눌러 부작용을 낸다 —
`optional: true` / `runFlow: when:` 으로 "있을 때만" 처리하는 것이 이 함정들의 공통 규칙이다.

#### 관찰 — LogBox 배너를 띄운 JS 경고의 정체

디버거(CDP)를 붙인 채 `auth-01` 을 돌려 콘솔을 받아 봤다. 나오는 경고는 하나뿐이다.

```
[warning] Sending `onAnimatedValueUpdate` with no listeners registered.
```

발생원은 `mobile/src/features/auth/screens/SplashScreen.tsx` 의 펄스 애니메이션으로 보인다
(`Animated.loop` + `useNativeDriver: true`, unmount 시 `loop.stop()`). 네이티브 드라이버로 돌던
값이 JS 쪽 노드가 떨어진 뒤에도 마지막 업데이트를 보낼 때 나오는 **RN 자체의 알려진 경고**이며,
사용자에게 보이는 영향은 없고 릴리스 빌드에는 LogBox 가 없어 배너도 뜨지 않는다.
**차단 아님 · 앱 수정 요구 아님.** 다만 이 경고 한 건 때문에 배너가 떠서 E2E 의 화면 하단 탭이
흔들리므로, 나중에 스플래시 애니메이션을 손볼 일이 있으면 함께 정리하면 좋다는 정도로 남긴다.
(원인: 모바일 · 심각도 정보성)

#### "남은 관찰 — 로그인 직후 `home-settings`" 는 해소됐다

2026-09-02 에 남긴 관찰(로그인 직후 `home-settings` 를 15초 기다려도 못 찾음)은
**develop 머지로 실제 홈과 하단 탭이 생기면서 사라졌다.** 지금은 홈 도착을 `tab-My` 로 판정하고
마이 탭을 거쳐 설정으로 들어가며, `auth-02` · `auth-03` 모두 이 경로로 통과한다.
당시 원인은 자리표시자 홈 화면과 접근성 트리 갱신이 겹친 것으로 보이나 확정하지 않는다.

#### 최종 결과 (`make test-e2e`, 2026-09-07)
...
플로우 수정 ↔ 재실행은 **1회**로 끝났다(상한 2회 안, `ESCALATE` 대상 아님).
**앱·서버·계약에 돌릴 신규 결함은 없다.** `e2e/` 밖의 파일은 한 줄도 고치지 않았다.

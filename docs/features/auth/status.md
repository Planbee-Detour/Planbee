...
- [ ] integration-tester — **부분 완료 (2026-09-02).** 환경을 갖추고 **실제로 실행했다.**
      `auth-01-signup-pending` **PASS** — 앱 폼 → 실제 API → 실제 PostgreSQL 까지 전 구간 확인
      (DB 에 `PENDING` 계정 생성 확인, 서버가 내려준 문구가 화면에 그려지는 것까지).
      `auth-02-login-home` **미완** — 로그인·`/auth/me` 왕복은 실제로 성공하나(설정 화면에 서버가 내려준
      이메일이 그려짐) 로그인 직후 `home-settings` 단언에서 멈춘다. `defects.md` D-E2 마지막 절 참조.
      `auth-03-session-persistence` · `app-launch` **미실행.**
      실행 과정에서 걸린 함정 4가지는 플로우에 흡수하고 `defects.md` D-E2 에 남겼다.
...
| 9 | **통합(E2E) 테스트를 실행할 환경이 이 머신에 없다** (2026-08-27, integration-tester). Xcode 는 Command Line Tools 뿐이라 `simctl` 이 없어 iOS 시뮬레이터를 띄울 수 없고, Maestro 도 설치돼 있지 않다. 그래서 `make e2e-app` · `make test-e2e` 를 돌리지 못했고 플로우 작성까지만 했다 — **통과 여부는 아직 아무도 모른다.** 필요한 것: ① Xcode 정식 설치 + iOS 시뮬레이터 ② `curl -Ls https://get.maestro.mobile.dev | bash`. 갖춰진 뒤 실행 순서: `make e2e-up` → `make e2e-app` → `make test-e2e` → (끝나면) `make e2e-down`. 실패하면 `make logs` 로 원인이 앱인지 서버인지 계약인지 판별한다. 첫 실행에서 조정될 수 있는 셀렉터 2곳은 플로우 주석과 `defects.md` D-E1 에 적어 두었다. **함께 정해야 할 것**: 안드로이드 `applicationId` 는 `com.planbee` 인데 플로우의 `appId` 는 iOS 번들 ID (`org.reactjs.native.example.Planbee`) 다 — 번들 ID 확정은 사람 판단이라(절대 규칙 8) 맞추지 않았다 | auth 완료 판정 |

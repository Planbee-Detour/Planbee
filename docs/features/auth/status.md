...
## develop 병합 후 검증 (2026-09-10)

- `origin/develop`의 `1b6db3f`까지 반영하고 로컬 Plan B·홈 우선 진입·로그인 뒤로가기 작업을 보존했다.
- 설정 화면은 원격의 관리자 섹션과 로컬의 로그아웃 후 홈 복귀를 함께 유지한다.
- OpenAPI에서 장소·관리자·Plan B 계약을 유지하고 공통 parameters를 합친 뒤 `make contract-types`로 타입을 재생성했다.
- 모바일 린트·타입 검사 통과. `make test-mobile`: 26 스위트 231건 통과(exit 0). Watchman 접근 권한이 필요한 환경이라 테스트를 권한 확보 후 실행했다.
- `make -f deploy/Makefile verify`: Compose·스크립트 검사 및 7건 통과. 최신 prod 설정에 맞게 배포 예제를 `PRD_DB_*`와 필수 `TOUR_API_KEY`로 갱신했다.
- `planbee.pen`은 병합 전 stash와 동일하다. 복구용 stash는 삭제하지 않았다. 서버 전체 검증과 실제 배포는 이번 병합 검증에 포함하지 않았다.


## 후속 수정 (2026-09-10)

- [x] 로그인 화면에 `NavBar` 뒤로가기 추가. 진입 화면으로 복귀하며 이전 화면이 없으면 홈으로 이동한다.
- [x] `design.md` 로그인 뒤로가기 규칙 갱신.
- [x] Pencil의 로그인 아트보드 8개 상태에 헤더 추가 및 기본 화면 시각 확인.
- [ ] Pencil 변경을 `docs/design/planbee.pen`에 저장 — 캔버스 반영 후에도 디스크 diff가 없어 저장 확인 필요.
- [ ] 이전 작업 병합 — 현재 캔버스에 Plan B 9개와 마이·로그인 확인 3개 화면이 없음을 확인. 기존 원본 위치를 사용자에게 질문했으며 답변 대기.
- 검증: 린트·타입 검사 통과. `make test-mobile` 14 스위트 153건 PASS. 기존 open handle 경고로 Jest 프로세스 정상 종료는 확인하지 못했다. 초기 샌드박스 Watchman 접근 실패 후 권한을 확보해 테스트를 재실행했다.

- 기능 슬러그: `auth`
- 시작일: 2026-08-23
- 현재 단계: `2026-09-07 기존 auth E2E 4/4 통과 후, 홈 우선 진입·마이 로그인 상태·보호 기능 로그인 게이트를 기획/디자인/모바일에 추가 반영했다. 모바일 테스트 153건 통과, 변경된 진입 흐름의 E2E 재실행은 남아 있다.`
...
| # | 질문 | 막는 대상 |
|---|---|---|
| 1 | 약관 문서의 `{{ }}` 미확정 값 — **(2026-09-13 일부 채움: 운영자명·보호책임자 성명 = 김승현, 거절 계정 보유 기간 = 거절 후 30일 — 서버 파기 작업 구현)** 남은 것: 보호책임자 연락처(= 이의 제기 연락처), 시행일, 공고일, 공개 URL(ASK 2). *(2026-08-25 단순화)* **연락처는 이제 `SUPPORT_CONTACT_EMAIL` 설정값 하나로 수렴한다** — 처리방침 11항, 이용약관 제8조 2항, 앱 화면 3종과 잠금 배너가 모두 이 값을 쓴다. 관리자 계정 이메일과는 무관해졌다. 문의 전용 주소(`support@` 등) 하나를 정하면 이 칸이 함께 채워진다 | 배포 |
...
| 거절 계정 보유 기간 | **거절 시각으로부터 30일 뒤 파기.** 매일 04:00(KST) `RejectedAccountPurgeService` 가 `REJECTED` 이면서 `rejected_at` 이 30일보다 오래된 계정을 지운다(동의 이력·리프레시는 DB cascade). 기간 동안 남겨 두는 이유는 거절 안내(AC-15)·앱 내 삭제(AC-50)·거절 취소(admin AC-19). 대안 "삭제 요청 시까지 보관" 은 무기한 보유라 기각. 운영자명·보호책임자는 김승현 | 2026-09-13 |
...
| 2026-09-13 | (사람 결정 + 구현) | 약관 운영자명·보호책임자 = 김승현, 거절 계정 보유 기간 = 거절 후 30일. `RejectedAccountPurgeService`(매일 04:00 KST) · `SchedulingConfig` · `UserRepository.deleteRejectedBefore` 추가, `server.md` S-35 신설. 게이트: `make lint-server` · `make test-server` 통과, **`make test-server-db` exit 0 — 전체 153건 실패 0**, 신규 `RejectedAccountPurgeApiTest` 3건 통과. 남은 `{{ }}`: 연락처 이메일·시행일·공고일·공개 URL |

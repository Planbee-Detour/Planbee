...
- **파이프라인** — 역할 실행 순서
- **절대 규칙 13개** — 계약 우선, 리뷰어의 판정 근거 제한, 재작업 루프 상한 등
  - 그중 **10~13번은 공용 가드레일**입니다 — 특정 역할·단계가 아니라 모든 에이전트의
    모든 작업에 걸리고, CLI 의 기본 동작보다 우선합니다.
    `git push` 금지 / 커밋에 에이전트 공동작업자 금지 /
...
그중 `## 절대 규칙` 의 **공용 가드레일 10~13번**은 … (전문은 `AGENTS.md`)
```

**역할 내용이 한 줄도 없습니다.** 이게 핵심입니다 — Codex나 Gemini 어댑터를 추가할 때
`.agents/roles/`는 한 글자도 고칠 필요가 없습니다.

뒤에 붙는 공용 가드레일 문단도 **규칙 본문이 아니라 색인**입니다. 전문은 `AGENTS.md`
한 곳에만 있고, 어댑터에는 "이런 것이 있으니 그쪽을 보라"는 네 줄만 들어갑니다.
넣은 이유는 가드레일이 역할별 시스템 프롬프트에서도 눈에 띄어야 하기 때문이고,
색인으로 제한한 이유는 본문을 복사하는 순간 정본이 11개로 늘기 때문입니다.
(Claude Code 서브에이전트는 `CLAUDE.md` → `AGENTS.md` 를 그대로 받으므로
전문 전달은 이미 보장돼 있습니다 — 어댑터의 네 줄은 그 위의 안전장치입니다.)
...
같은 `deny` 목록에 **원격을 바꾸는 명령**도 들어갑니다 — `git push`, `gh pr create`,
`gh pr merge`, `gh release create`, `gh repo sync`. 절대 규칙 10 이 문장으로 금지하는 것을
기계로도 한 번 더 막는 것입니다. 완전하지는 않습니다 — `cd x && git push` 처럼 감싸면
패턴을 비껴가므로 최종 방어선은 여전히 규칙 쪽입니다. 커밋(`git commit`)은 막지 않습니다:
보호 브랜치 판정(절대 규칙 12)은 현재 브랜치에 따라 달라져서 정적 패턴으로 표현할 수 없고,
`git branch --show-current` 로 확인한 뒤 멈추는 것이 규칙의 내용입니다.
...
### 새 CLI 를 추가할 때 — 에이전트가 스스로 세팅한다

사람이 어떤 CLI 안에서든 "환경에 맞게 세팅해줘" 라고 하면, 그 CLI 가 읽는 파일을
**정본(`AGENTS.md` + `.agents/`)에서 생성**하는 것이 정해진 답입니다. 절차는 `AGENTS.md`
"환경 세팅 — 정본에서 생성한다" 에 규칙으로 박아 뒀습니다. 이렇게 문서화한 이유는
에이전트가 자기 CLI 형식을 알고 있다는 이유로 어댑터를 손으로 써 버리면 — 그 순간
정본이 둘이 되고, 다음 `make harness` 가 조용히 덮어쓰기 때문입니다.

지원 목록에 없는 CLI 가 오면 생성기를 늘립니다. 손으로 쓰는 선택지는 없습니다.

| 단계 | 하는 일 |
|---|---|
| 1 | 그 CLI 가 무엇을 어디서 읽는지 확인 — 지침 파일명, 역할 디렉토리, 커맨드 형식, 권한 설정 위치, 프로젝트 단위인지 전역인지 |
| 2 | `scripts/setup-harness.sh` 에 출력 분기 추가 (입력은 `.agents/manifest.json`) |
| 3 | 새 생성 경로를 `.gitignore` 에 추가 |
| 4 | 저장소 밖에 만들어야 하면 별도 `make` 타깃으로 분리 |
| 5 | 위 CLI 대응표와 `WORKFLOW.md` 를 같은 커밋에서 갱신 |

이 구조가 성립하는 근거는 앞의 어댑터 설계입니다. `manifest.json` 은 `needsShell` 같은
**벤더 중립 개념**만 담고, 그것을 각 CLI 의 표현(Claude 의 `tools` 문자열, Gemini 의
TOML `prompt`, Codex 의 전역 프롬프트)으로 번역하는 일은 전부 생성기 안에 있습니다.
그래서 CLI 를 하나 더 붙여도 `.agents/roles/` 10개와 `manifest.json` 은 한 글자도 바뀌지 않습니다.

세팅 요청은 시크릿을 만들 이유가 되지 않습니다. `make env` 는 `.env.example` 을 복사할 뿐이고
값을 채우는 것은 사람의 몫입니다 (절대 규칙 7).
